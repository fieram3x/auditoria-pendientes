...
/* LISTA PROFESIONAL TIPO TARJETAS COMPACTAS */
.report-card {
...
.inc-actions div[data-testid="stPopover"] button {
...
/* Oculta la X nativa del diálogo. Usar Cerrar/Cancelar para limpiar estado. */
...
def render_report_table(data, dff):
    ...
    for _, row in dff.reset_index().iterrows():
        ...
        desc_short = desc if len(desc) <= 160 else desc[:157] + "..."
        ...
        c1, c2, c3, c4, c5, c6, c7, c8, c9 = st.columns([1.05, .9, .8, 1.15, 1.45, .8, .95, 2.2, .55])

        with c1:
            st.markdown(f'<div class="inc-label">ID</div><div class="inc-value">{rid}</div>', unsafe_allow_html=True)
        with c2:
            st.markdown(f'<div class="inc-label">Fecha</div><div class="inc-value">{safe_date(row["Fecha Creación"])}</div>', unsafe_allow_html=True)
        with c3:
            st.markdown(f'<div class="inc-label">Hotel</div><div class="inc-value">{hotel_short}</div>', unsafe_allow_html=True)
        with c4:
            st.markdown(f'<div class="inc-label">Departamento</div><div class="inc-value">{row["Departamento"]}</div>', unsafe_allow_html=True)
        with c5:
            st.markdown(f'<div class="inc-label">Tipo</div><div class="inc-value">{row["Tipo de Incidencia"]}</div>', unsafe_allow_html=True)
        with c6:
            st.markdown('<div class="inc-label">Prioridad</div>', unsafe_allow_html=True)
            st.markdown(badge(row["Prioridad"]), unsafe_allow_html=True)
        with c7:
            st.markdown('<div class="inc-label">Estatus</div>', unsafe_allow_html=True)
            st.markdown(badge(row["Estatus"]), unsafe_allow_html=True)
        with c8:
            st.markdown(f'<div class="inc-label">Descripción</div><div class="inc-desc">{desc_short}</div>', unsafe_allow_html=True)
        with c9:
            st.markdown('<div class="inc-label" style="text-align:right;">Acciones</div>', unsafe_allow_html=True)
            st.markdown('<div class="inc-actions">', unsafe_allow_html=True)
            with st.popover("⋮⌄"):
    ...
    if active:
        if active.get("type") == "edit":
            open_edit_dialog(data, active.get("id"), estados)
        elif active.get("type") == "bitacora":
            open_bitacora_dialog(data, active.get("id"))


def open_edit_dialog(data, edit_id, estados):
    df = data["Pendientes"]
...
def open_bitacora_dialog(data, bit_id):
